 ...
 * Date: July 14 2016
 */
var game = new Phaser.Game(800,600, Phaser.AUTO, 'space-Fighter', {preload: preload, create: create, update: update, render: render});

var player; //intializing player
var greenEnemies; //intializing enemyA {green enemy}
var blueEnemies; //initializing enemyB {blue enemy}
var enemyBullets; //intializing enemy bullets
var starfield; //intializing bg 'starfield'
var cursors;
var bank;
var shipTrail; //shiptrail ~ the engines behind the player ship
var explosions; //explosions
var playerDeath; //basically an explosion
var bullets;
var fireButton;
var bulletTimer = 0;
var shields;
...
var greenEnemySpacing = 1000; // I should have decreased this ~ it's dependant on how the player moves through the scene
var blueEnemyLaunchTimer;
var blueEnemyLaunched = false;
var blueEnemySpacing = 2500; // this should have been increased as it adds more diffculty to the game
var bossLaunchTimer;
var bossLaunched = false; //Once the boss has been called this will change to true
var bossSpacing = 20000; //This value should be decreased but, it might actually make the game too easy to complete
...
//All of the following images are free
...
function preload() {
    ...
    game.load.bitmapFont('spacefont', '../assets/spacefont/spacefont.png', 'assets/spacefont/spacefont.xml'); //free font 
...
function create() {
    ...
    starfield = game.add.tileSprite(0, 0, 1200, 1900, 'starfield'); //background scroll

    //  phaser and bullet physics
    ...
    // Player ~ 
    player = game.add.sprite(400, 500, 'ship'); //add player ship
    player.health = 100; // given value to player health ~ pretty generous value considering the player also has a shield value
    player.anchor.setTo(0.5, 0.5);
    game.physics.enable(player, Phaser.Physics.ARCADE);
    player.body.maxVelocity.setTo(MAXSPEED, MAXSPEED);
    player.body.drag.setTo(DRAG, DRAG);
    player.weaponLevel = 1 // automatically level 1 upon reseting the game, can change to level 2 
    player.events.onKilled.add(function(){ //When the player dies
        shipTrail.kill(); //stop the player's ship trail
    });
    ...
    //  Blue enemy's bullets settings ~ i may tweak this if it is too dificult for the player to evade it's bullets
    ...
    // EnemyB  ~the most overpowered enemies in the game
    blueEnemies = game.add.group();
    blueEnemies.enableBody = true;
    blueEnemies.physicsBodyType = Phaser.Physics.ARCADE; // Using Phaser 
    ...
    //BOSS ~ might be too overpowered
    boss = game.add.sprite(0, 0, 'boss');
    boss.exists = false;
    boss.alive = false;
    boss.anchor.setTo(0.5, 0.5);
    boss.damageAmount = 50; // boss damage, two hits and it's game over for the player
    boss.angle = 180;
    boss.scale.x = 0.6;
    boss.scale.y = 0.6;
    ...
    boss.finishOff = function() {
        if (!boss.dying) {  //boss conditions on death
            boss.dying = true;
            bossDeath.x = boss.x;
            bossDeath.y = boss.y;
    ...
    boss.fire = function() {
        if (game.time.now > bossBulletTimer) {
            ...
            function chargeAndShoot(side) {
                ...
                ray.alpha = 0;
                ray.scale.y = 13;
                game.add.tween(ray).to({alpha: 1}, chargeTime, Phaser.Easing.Linear.In, true).onComplete.add(function(ray){ //Charge time may need to be configured
                    ray.scale.y = 150;
    ...
    boss.update = function() { //update boss' state
      ...
      //  Squish and rotate boss for illusion of "realistic motion" //added reqiurement within assignment documentation*
      ...
      //  fire if player is in target ~ May need to slow down the attack speed of the boss
    ...
    //  Player may use the spacebar, for firing thir weapon
...
function fireBullet() {
    switch (player.weaponLevel) {
        ...
        {
            var BULLET_SPEED = 400; //player bullet speed
            var BULLET_SPACING = 250; //player bullet spacing ~ affects the distance of the bullet between the previous bullet fired
            ...
            {
                //  And fire it
                //  Make bullet come out of tip of ship with right angle //specifiying how the bullet will be called from
                var bulletOffset = 20 * Math.sin(game.math.degToRad(player.angle));
                bullet.reset(player.x + bulletOffset, player.y); //using the players cordinate
                bullet.angle = player.angle;
                game.physics.arcade.velocityFromAngle(bullet.angle - 90, BULLET_SPEED, bullet.body.velocity);
                bullet.body.velocity.x += player.body.velocity.x;

                bulletTimer = game.time.now + BULLET_SPACING; //Bullet spacing affects the game dramatically as the game relies on bullets from the player to generate certain 
                                                              // conditions within the game, might increase the bullet speed to make it easier on the player                               
            }
        }
        break;
...
function launchGreenEnemy() { //generate green enemy
    var ENEMY_SPEED = 300;
...
function shipCollide(player, enemy) { //If player hits enemy or vice -versa
    enemy.kill(); //counts as enemy kill

    player.damage(enemy.damageAmount); //but player takes hit damage
    shields.render(); //and shield damage

    if (player.alive) { // if player remains alive
        var explosion = explosions.getFirstExists(false);
...
function hitEnemy(enemy, bullet) { //Player hits an enemy conditions
    var explosion = explosions.getFirstExists(false);
    explosion.reset(bullet.body.x + bullet.body.halfWidth, bullet.body.y + bullet.body.halfHeight);
    explosion.body.velocity.y = enemy.body.velocity.y;
    explosion.alpha = 0.7;
    explosion.play('explosion', 30, false, true); //allow explosion
    if (enemy.finishOff && enemy.health < 5) {
      enemy.finishOff();
    } else {
        enemy.damage(enemy.damageAmount); //hit damage 
    }
    bullet.kill();
    ...
    //  Blue enemies come in after a score of 1000 ~ To show player advancement
    ...
    //  Launch boss ~ with new enemyA & enemyB conditions
    ...
    //  Weapon upgrade //If player score is 5000 & player weapon is level 2, intialize wepon upgrade
...
function restart () { //restart game
    ...
    player.revive();
    player.health = 100;
    shields.render(); //shields are set to 100 
    score = 0;
    scoreText.render();
